...
import { FaCircleInfo, FaPenClip, FaLock, FaTv, FaCheck, FaRegCircleXmark, FaFileInvoiceDollar, FaRegCreditCard, FaFileImport } from "react-icons/fa6";
...
export default function PaymentList({ ...props }) {
    ...
    const handleEdit = (e) => {
        e.preventDefault();
        router.post('/client/update-payment',data);
        closeModal();
    };
    ...
    const handleDelet = (RequestID) => {
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                router.post(`/client/delet-payment/${RequestID}`,
                    {
    ...
    const handleExchange = (e) => {
        ...
        if (name === 'amount' && data.currency) {
            fetchExchangeRate(data.currency, auth.user.currency, value);
        } else if (name === 'currency' && data.amount) {
            fetchExchangeRate(value, auth.user.currency, data.amount);
        }
    };
    ...
    //     console.log('Equivalence:', `${amount} ${fromCurrency}`);
    //     setData({ equivalence: `${amount} ${toCurrency}` });
    // }
    const fetchExchangeRate = async (fromCurrency, toCurrency, amount) => {
        try {
            ...
            if (data && data.conversion_rates && data.conversion_rates[toCurrency]) {
                ...
                setData(prevData => ({ ...prevData, equivalence: `${convertedAmount} ${toCurrency}` }));
            } else {
    ...
    const handleSearch = (e) => {
        ...
        router.post(route('client.getPayment'),
            payload,
            {
                preserveScroll: true,
                preserveState: true,
                        ...
                        <div className="p-4 overflow-y-auto">
                            <form onSubmit={handleEdit} >
                                <div className="mb-4">
                                    ...
                                    <input type="number" onChange={handleChange} value={data.amount} name='amount' className="peer py-3 px-4 ps-11 block w-full bg-gray-100 border-transparent rounded-lg text-sm focus:border-blue-500 focus:ring-blue-500 disabled:opacity-50 disabled:pointer-events-none dark:bg-gray-700 dark:border-transparent dark:text-gray-400 dark:focus:ring-gray-600" />
                                </div>
                                <div className="mb-4">
                                    <label className="block text-gray-700 text-sm font-semibold mb-2">type</label>
                                    <select onChange={handleChange} name='type' value={data.type} className="py-3 px-4 pe-9 block w-full border-gray-200 rounded-lg text-sm focus:border-purple-600 focus:ring-purple-600 disabled:opacity-50 disabled:pointer-events-none dark:bg-slate-900 dark:border-gray-700 dark:text-gray-400 dark:focus:ring-gray-600">
                                ...
                                <div className="mb-4">
                                    ...
                                    <select onChange={handleExchange} name='currency' value={data.currency} className="py-3 px-4 pe-9 block w-full border-gray-200 rounded-lg text-sm focus:border-purple-600 focus:ring-purple-600 disabled:opacity-50 disabled:pointer-events-none dark:bg-slate-900 dark:border-gray-700 dark:text-gray-400 dark:focus:ring-gray-600">
                                        <option >Open this select menu</option>
                                ...
                                <div className="mb-4">
                                    <label className="block text-gray-700 text-sm font-semibold mb-2"> Equivalence in {auth.user.currency}</label>
                                    <input name='equivalence' id="equivalence" type="text" value={data.equivalence} readOnly className="appearance-none block w-full bg-grey-lighter text-grey-darker focus:border-purple-600 focus:ring-purple-600 border border-grey-lighter rounded py-3 px-4 mb-3" />
                                </div>
                                <div className="mb-4">
                                    <label className="block text-gray-700 text-sm font-semibold mb-2">Preuve</label>
                                    <input name='preuve' id="pruve" type="file" onChange={handleFile} className="appearance-none block w-full bg-grey-lighter text-grey-darker focus:border-purple-600 focus:ring-purple-600 border border-grey-lighter rounded py-3 px-4 mb-3" />
                                </div>
